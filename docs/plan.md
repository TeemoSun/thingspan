...
| id | 主键 |
| name | 类别名称 |
| has_warranty | 勾选「保修期」：资产可填写保修月数，按购买日自动推算保修结束日期 |
...
| warranty_months | 保修月数（可空，类别勾选「保修期」时填写，不同资产可不同） |
| warranty_end_date | 保修结束日期（按购买日 + 月数×30 天自动推算） |
...
- 保修期由资产填写保修月数（不同资产可不同），结束日期按「购买日 + 月数×30 天」自动推算，新建或改购买日/月数时重算，月数置空则清空
