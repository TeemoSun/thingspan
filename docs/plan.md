...
| name | 类别名称 |
| icon | Tabler 图标名 |
| template | 模板类型：`product`（数码）/ `membership`（会员）/ `other`（其他） |
| fields | 自定义字段定义 JSON：`[{key, name, type}]`，type ∈ text/date/number |
| created_at | 创建时间 |
...
| 模板 | 系统字段 | 可选状态 |
|---|---|---|
| product（数码） | 品牌、型号、序列号、保修结束日期 | 使用中 / 已售出 / 已损坏 |
| membership（会员） | 到期日期 | 使用中 / 已过期 |
| other（其他） | — | 使用中 / 已售出 / 已损坏 |
...
| category_id | 所属类别 |
| name | 名称 |
| brand / model / serial_number | 品牌 / 型号 / 序列号（可空） |
| purchase_date | 购买日期（必填） |
| purchase_price | 购买价格（必填） |
| warranty_end_date | 保修结束日期（可空，数码类） |
| expiry_date | 到期日期（可空，会员类） |
| status | `in_use` / `sold` / `broken` / `expired`，默认 `in_use` |
| sale_date / sale_price | 售出日期 / 售出价格（已售出时必填） |
| broken_date | 损坏日期（已损坏时必填） |
| notes | 备注 |
| custom_values | 自定义字段值 JSON |
| created_at / updated_at | 时间戳 |

细节：

- 保修期按「购买日 + 保修月数」自动推算结束日期，可手动修改（类别模板配置保修月数）
- 售出 / 损坏操作要求填写对应日期，售出还需售出价格
- 到期日过期的「使用中」资产由每日任务自动标记为 `expired`
...
| 资产详情 | 按类别字段动态渲染表单；状态切换（售出 → 填日期与售价；损坏 → 填日期）；成本展示 |
| 类别管理 | 创建/编辑类别，配置模板与自定义字段 |
| 提醒记录 | 查看发送历史，手动忽略 |
